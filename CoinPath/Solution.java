 ...
 * if this place can be jumped to. Also, if you step on the index i, you have to pay Ai coins. If Ai is -1,
    ...
    public List<Integer> cheapestJump(int[] A, int B) {
        ...
        for (int i = 0;i < len;++ i) {
            ...
            for (int j = Math.max(0, i - B);j < i;++ j) {
                ...
                if (val < costs[i] || (val == costs[i] && lens[i] < lens[j] + 1)) {
                    costs[i] = val;
                    preIndexes[i] = j;
                    lens[i] = lens[j] + 1;
                }
        ...
        for (int idx = len - 1;idx >= 0;idx = preIndexes[idx]) {
            path.add(0, idx + 1);
        }
